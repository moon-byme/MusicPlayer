 ...
 * @author Isabelle
 */
public class ArvoreAVL implements IArvore<Musicas> {
    
    private NoAVL raiz; //nosso root
    private int tamanho; //quantidade de músicas
    
    private class NoAVL {
        Musicas musica; //dado usado: música
    ...
    private NoAVL inserirRecursivo(NoAVL atual, Musicas musica) {
        if (atual == null) {//posição de inserção vazia
            tamanho++;
            return new NoAVL(musica);
        }
        
        //compara pelo nome da música
        int cmp = musica.getNome().compareToIgnoreCase(atual.musica.getNome());
        
        if (cmp < 0)
            atual.esquerda = inserirRecursivo(atual.esquerda, musica);//se o nome menor
        else if (cmp > 0)
            atual.direita = inserirRecursivo(atual.direita, musica);//se o nome maior
        else
    ...
    @Override
    public boolean buscar(Musicas musica) {
        return buscarRecursivo(raiz, musica);
    }
    
    private boolean buscarRecursivo(NoAVL atual, Musicas musica) {
     ...
     */
    public Musicas buscarPorTitulo(String titulo) {
        return buscarPorTituloRecursivo(raiz, titulo);
    }
    
    private Musicas buscarPorTituloRecursivo(NoAVL atual, String titulo) {
        if (atual == null) return null;
        
        int cmp = titulo.compareToIgnoreCase(atual.musica.getNome());
    ...
    @Override
    public void remover(Musicas musica) {
        if (musica == null) return;
        raiz = removerRecursivo(raiz, musica);
    }
    
    private NoAVL removerRecursivo(NoAVL atual, Musicas musica) {
        if (atual == null) return null;
    ...
    @Override
    public List<Musicas> inOrder() {
        List<Musicas> lista = new ArrayList<>();
        inOrderRecursivo(raiz, lista);
        return lista;
    }
    
    private void inOrderRecursivo(NoAVL no, List<Musicas> lista) {
        if (no != null) {
